...
async function buildFilters() {
  ...
  const genreData = await fetchApi(apiUrl("/genre/tv/list"));
...
async function loadSeries() {
  ...
  const data = await fetchApi(apiUrl("/discover/tv", params));
  isLoading = false;
  ...
  if (data.results.length === 0) {
    grid.innerHTML =
      '<div class="no-results"><h3>No series found</h3><p>Try different filters</p></div>';
    return;
  }
...
function renderSeries() {
  ...
  grid.innerHTML = "";
  cachedResults.slice(0, getCardCount("list")).forEach((show) => {
    grid.appendChild(createCard(show, "tv"));
  });
}
...
const handleResize = debounce(renderSeries, 150);
